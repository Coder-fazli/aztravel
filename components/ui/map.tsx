...
export function WorldMap({
  ...
  const svgMap = useMemo(
    () => map.getSVG({ radius: 0.22, color: "#00000040", shape: "circle", backgroundColor: "white" }),
    [map]
  );
  ...
  return (
    <div className="w-full aspect-[2/1] dark:bg-black bg-white rounded-lg relative font-sans overflow-hidden">
      ...
      >
        <defs>
          <linearGradient id="evisa-path-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="white" stopOpacity="0" />
            <stop offset="5%" stopColor={lineColor} stopOpacity="1" />
            <stop offset="95%" stopColor={lineColor} stopOpacity="1" />
            <stop offset="100%" stopColor="white" stopOpacity="0" />
          </linearGradient>
          <filter id="evisa-glow">
          ...
          return (
            <g key={`path-group-${i}`}>
              <motion.path
                ...
                fill="none"
                stroke="url(#evisa-path-gradient)"
                strokeWidth="1"
              ...
              {loop && (
                <motion.circle
                  ...
                  animate={{ offsetDistance: [null, "0%", "100%", "100%", "100%"] as unknown as string[], opacity: [0, 0, 1, 0, 0] }}
          ...
          return (
            <g key={`points-group-${i}`}>
              <g>
                ...
                >
                  <circle cx={startPoint.x} cy={startPoint.y} r="3" fill={lineColor} filter="url(#evisa-glow)" className="drop-shadow-lg" />
                  ...
                  >
                    <foreignObject x={startPoint.x - 50} y={startPoint.y - 35} width="100" height="30" className="block">
                      <div className="flex items-center justify-center h-full">
                        <span className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 tracking-wide">{dot.start.label}</span>
                      </div>
                    </foreignObject>
                  </motion.g>
                )}
              ...
              <g>
                <motion.g
                  ...
                  transition={{ type: "spring", stiffness: 400, damping: 10 }}
                >
                  <circle cx={endPoint.x} cy={endPoint.y} r="3" fill={lineColor} filter="url(#evisa-glow)" className="drop-shadow-lg" />
                ...
                {showLabels && dot.end.label && (
                  ...
                  >
                    <foreignObject x={endPoint.x - 50} y={endPoint.y - 35} width="100" height="30" className="block">
                      <div className="flex items-center justify-center h-full">
                        <span className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 tracking-wide">{dot.end.label}</span>
                      </div>
                    </foreignObject>
                  </motion.g>
                )}
